...
class TiledCamera(Camera):
    ...
    Warp backend: PhysX runs simulation and Newton/Warp perform ray tracing; PhysX→Newton
    ...
    def _initialize_impl(self):
        ...
        if _renderer_type in ("warp_renderer", "newton_warp"):  # newton_warp kept for backward compat
            logger.info(
                "TiledCamera %s: using renderer backend warp_renderer (from cfg.renderer_type=%s)",
                self.cfg.prim_path,
                self.cfg.renderer_type,
            )
            # Use Warp renderer (Newton Warp implementation)
            ...
            if renderer_cls is None:
                raise RuntimeError("Failed to load warp_renderer class.")
    ...
    def _update_buffers_impl(self, env_ids: Sequence[int]):
        ...
        # Warp path: sync PhysX→Newton then render; whole block timed as newton_warp_sync_plus_render
        if self._renderer is not None:
            from isaaclab.sim._impl.newton_manager import NewtonManager
